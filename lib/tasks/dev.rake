namespace :dev do
  ...
  task setup: :environment do
    if Rails.env.development?
      show_spinner('Deleting DB...') { `rails db:drop` }
      show_spinner('Create DB...') { `rails db:create` }
      show_spinner('Migrating DB...') { `rails db:migrate` }
      show_spinner('Cadastrando o adminstrador padrão...') { `rails dev:add_default_admin` }
      show_spinner('Cadastrando administradores extras...') { `rails dev:add_extras_admins` }
      show_spinner('Cadastrando o usuário padrão...') { `rails dev:add_default_user` }
      show_spinner('Cadastrando assuntos padroes...') { `rails dev:add_subjects` }
  ...
  task add_answers_to_redis: :environment do
    show_spinner("Add all answers to redis...") do
